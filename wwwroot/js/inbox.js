...
function renderFormRow(item) {
    ...
    if (isApproval && isPending && (userRole === 'Admin' || userRole === 'Manager')) {
        actions =
            '<button class="btn btn-sm btn-success ui-act-btn" onclick="approveForm(' + item.id + ')" title="اعتماد">' +
                '<i class="bi bi-check-lg"></i></button> ' +
            '<button class="btn btn-sm btn-danger ui-act-btn" onclick="rejectForm(' + item.id + ')" title="رفض">' +
                '<i class="bi bi-x-lg"></i></button> ' +
            '<button class="btn btn-sm btn-outline-primary ui-act-btn" onclick="viewForm(' + item.formId + ',' + item.id + ')" title="معاينة">' +
                '<i class="bi bi-eye"></i></button> ' +
            '<button class="btn btn-sm btn-outline-danger ui-act-btn" onclick="downloadPdf(' + item.formId + ')" title="PDF">' +
                '<i class="bi bi-file-earmark-pdf"></i></button>';
    } else if (!isApproval && isPending) {
        actions =
            '<button class="btn btn-sm btn-primary ui-act-btn" onclick="fillForm(' + item.formId + ',' + item.id + ')" title="تعبئة">' +
                '<i class="bi bi-pencil-fill"></i></button> ' +
            '<button class="btn btn-sm btn-outline-primary ui-act-btn" onclick="viewForm(' + item.formId + ',' + item.id + ')" title="معاينة">' +
                '<i class="bi bi-eye"></i></button>';
    } else {
        actions =
            '<button class="btn btn-sm btn-outline-primary ui-act-btn" onclick="viewForm(' + item.formId + ',' + item.id + ')" title="عرض">' +
                '<i class="bi bi-eye"></i></button>';
        if (isApproval) {
            actions += ' <button class="btn btn-sm btn-outline-danger ui-act-btn" onclick="downloadPdf(' + item.formId + ')" title="PDF">' +
                '<i class="bi bi-file-earmark-pdf"></i></button>';
        }
    }
...
function renderRequestRow(item) {
    ...
    var actions = '<button class="btn btn-sm btn-primary ui-act-btn" title="' + (isPending ? 'مراجعة' : 'تفاصيل') + '" onclick="ibOpenRequest(' + assignmentId + ')">' +
        '<i class="bi bi-' + (isPending ? 'check2-square' : 'eye') + '"></i></button>';
