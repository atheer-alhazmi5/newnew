...
// ─── LOAD / RENDER TABLE ──────────────────────────────────────────────────────
// ─── STEP 1 ORGANIZATIONAL UNIT TREE SELECTOR ─────────────────────────────
let fdStep1OuExpanded = {};

function fdStep1OuTogglePanel() {
    const panel = document.getElementById('fdStep1OuPanel');
    const trig = document.getElementById('fdStep1OuTrigger');
    if (!panel) return;
    if (panel.classList.contains('d-none')) {
        const cur = document.getElementById('fdStep1OuInput')?.value;
        if (cur) fdStep1OuExpandAncestorsForSelection(parseInt(cur, 10));
...
function fdRenderStep1OuTreeRows(byParent, parentKey, depth, selectedId, expandedMap) {
    const rows = byParent[parentKey] || [];
    const sel = selectedId !== undefined && selectedId !== null ? String(selectedId) : (document.getElementById('fdStep1OuInput')?.value || '');
    let html = '';
    rows.forEach(u => {
        const idStr = String(u.id);
        const children = byParent[idStr] || [];
...
function fdStep1OuSetSelection(id, name) {
...
function fdStep1Html(d) {
    ...
    // ws options removed - using tree selector instead
...
function fdCollect1() {
    ...
    return {
        ...
        ouId:     parseInt(document.getElementById('fdStep1OuInput')?.value||'0'),
    ...
    try {
        ...
        let html = `<div class="fd-section">
            ...
            <div class="fd-detail-grid">
                ...
                <span class="fd-detail-lbl">الوحدة التنظيمية المالكة</span><span class="fd-detail-val">${esc(d.orgUnitName)}</span>
