...
function clsNormalizeColorHex(h) {
    if (h == null) return '';
    h = ('' + h).trim();
    if (!h) return '';
    if (h[0] !== '#') h = '#' + h;
...
function clsSelectPresetColor(el, mode) {
    var q = mode === 'add' ? '#clsAddColorPicker .cls-color-swatch' : '#clsEditColorPicker .cls-color-swatch';
    document.querySelectorAll(q).forEach(function (e) { e.classList.remove('selected'); });
    el.classList.add('selected');
    var hex = el.getAttribute('data-color') || '#25935F';
    if (mode === 'add') {
        document.getElementById('clsAddColor').value = hex;
        document.getElementById('clsAddColorHex').textContent = hex;
    } else {
        document.getElementById('clsEditColor').value = hex;
        document.getElementById('clsEditColorHex').textContent = hex;
        var cw = document.getElementById('clsEditColorCustomWrap');
...
function clsShowAddModal() {
    ...
    document.querySelectorAll('#clsAddColorPicker .cls-color-swatch').forEach(function (el) {
        el.classList.toggle('selected', (el.getAttribute('data-color') || '').toUpperCase() === '#25935F');
    });
...
function clsShowEditModal(id) {
    ...
    var hexNorm = clsNormalizeColorHex(cls.color);
    document.getElementById('clsEditColor').value = cls.color;
    document.getElementById('clsEditColorHex').textContent = cls.color;
    var found = false;
    document.querySelectorAll('#clsEditColorPicker .cls-color-swatch').forEach(function (el) {
        var d = clsNormalizeColorHex(el.getAttribute('data-color'));
        var ok = d === hexNorm;
        el.classList.toggle('selected', ok);
        if (ok) found = true;
    });
    var cw = document.getElementById('clsEditColorCustomWrap');
    var ci = document.getElementById('clsEditColorCustom');
    if (cw && ci) {
        if (!found) {
            cw.classList.remove('d-none');
            var cv = /^#[0-9a-fA-F]{6}$/.test(hexNorm) ? hexNorm : '#25935F';
            ci.value = cv;
            ci.oninput = function () {
                document.getElementById('clsEditColor').value = this.value;
                document.getElementById('clsEditColorHex').textContent = this.value;
                document.querySelectorAll('#clsEditColorPicker .cls-color-swatch').forEach(function (e) { e.classList.remove('selected'); });
            };
        } else {
